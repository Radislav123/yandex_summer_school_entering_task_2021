...
## На что обратить внимание (из задания)
...
Папка для интерпретера - [*venv/*](venv)
...
## Не модули, но тоже присутствует
...
1) тут написать, как создать и подключить БД
2) для инициализации БД необходимым, для корректной работы api, минимумом данных,
требуется выполнить следующую команду:  
`python manage.py runscript add_courier_types_to_db`
